...
static void network_disconnected(void)
{
	...
	fprintf(stderr,"This machine has been disconnected from the internet\n");
...
{
  if(!strcmp(sender, *mittente)){
    fprintf(stderr, "Received message from %s!\n", sender);
    fprintf(stderr, "Message: %s\n", message);
    printf("%s\n", message);
    fflush(stdout);
  }
  else{
    fprintf(stderr, "Received message from %s. Ignoring\n", sender);
  }
}
...
int main(int argc, char *argv[])
{
  ...
  if(argc!=4){
    fprintf(stderr, "Nope, you have to specify the desired person, username and password\n");
    return -1;
  }
  mittente=&(argv[1]);
